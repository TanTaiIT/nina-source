<?php
	class Cart
	{
		...
		public function get_gia($idpro,$mau=0,$size=0)
		{

			if($mau > 0 || $size > 0){

				$laygia = $this->d->rawQueryOne("select giaban as gia,giamoi from #_product_thuoctinh where type = 'san-pham' and id_mau = ? and id_size = ? and id_product = ? limit 0,1",array($mau,$size,$idpro));

			}else{

				$laygia = $this->d->rawQueryOne("select gia,giamoi from #_product where type = 'san-pham' and id = ? limit 0,1",array($idpro));
		...
		public function get_order_total()
		{
			...
			if(isset($_SESSION['cart']))
			{
				...
				for($i=0;$i<$max;$i++)
				{
					...
					$laygia = $this->get_gia($pid,$mau,$size);
